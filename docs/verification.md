...
| `make kind-scoped-e2e` | A chart configured for one namespace receives matching manager RBAC, keeps tenant author permissions namespaced, and does not reconcile an out-of-scope resource | A complete hostile-tenant boundary or production admission policy |
